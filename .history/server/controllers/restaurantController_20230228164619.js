...
const MenuItem = require('../models/restaurantmodels');

// Function to retrieve popular dishes for restaurants in an area
 async function getPopularDishesInArea(area) {
  const restaurants = await axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${area}&radius=500&type=restaurant&key=YOUR_API_KEY`);
  const popularDishes = {};

  // Loop through each restaurant and get its menu items
  for (let i = 0; i < restaurants.length; i++) {
    const restaurant = restaurants[i];
    const restaurantDetails = await axios.get(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${restaurant.place_id}&fields=name,menu&key=YOUR_API_KEY`);
...
module.exports= getPopularDishesInArea